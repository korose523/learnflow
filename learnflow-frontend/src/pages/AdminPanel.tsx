...
export default function AdminPanel() {
  ...
  if (error) return (
    <div style={{ textAlign: 'center', padding: 80 }}>
      ...
      <button className="btn btn-primary" onClick={loadOverview}>重试</button>
    </div>
  );
      ...
      {tab === 'overview' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          <div className="card" style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 42, fontWeight: 700, color: '#6366f1' }}>{stats?.total_users || 0}</div>
                ...
                <div key={task.id} style={{ padding: 16, border: '1px solid #e2e8f0', borderRadius: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: 8 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button className="btn" style={{ background: '#f0fdf4', color: '#22c55e', padding: '6px 12px', fontSize: 13 }} onClick={() => handleReview(task.id, true)}>
                        <CheckCircle2 size={14} /> 通过
                      </button>
                      <button className="btn" style={{ background: '#fef2f2', color: '#ef4444', padding: '6px 12px', fontSize: 13 }} onClick={() => handleReview(task.id, false)}>
                        <XCircle size={14} /> 拒绝
                      </button>
                    </div>
                  </div>
      ...
      {tab === 'scripts' && (
        <div className="card">
          ...
          <form onSubmit={handleCreateScript} style={{ display: 'grid', gap: 10, marginBottom: 20, padding: 16, background: '#f8fafc', borderRadius: 10 }}>
            ...
            <button type="submit" className="btn btn-primary" style={{ padding: '8px 16px' }}>创建</button>
              ...
              {scripts.map((s) => (
                <tr key={s.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '8px' }}>
                    <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 11, background: '#eef2ff', color: '#6366f1', fontWeight: 600 }}>
                      {s.category}
                    </span>
                  </td>
