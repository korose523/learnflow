...
import { BookOpen, GraduationCap, Users, Shield, LogOut, Activity, FileText, Trophy } from 'lucide-react';
...
const navItems = [
  ...
  { path: '/student/leaderboard', label: '学习榜', icon: Trophy, role: 'student' },
  ...
  { path: '/parent/summary', label: '孩子详情', icon: FileText, role: 'parent' },
...
export default function Layout() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'transparent' }}>
      ...
      <AmbientOrbs />
      <header className="lf-app-header" style={{
        ...
        {/* 左侧 Logo（童趣圆润 + 渐变芯片） */}
        <Link className="lf-header-brand" to={`/${user?.role || 'student'}`} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 10 }}>
          <span
            style={{
          ...
          >🎓</span>
          <span className="lf-gradient-text lf-header-brand-text" style={{ fontSize: 21, fontWeight: 800, fontFamily: "'Baloo 2','Nunito','Noto Sans SC',sans-serif" }}>LearnFlow</span>
        </Link>

        {/* 中间导航 */}
        <nav className="lf-header-nav" aria-label="主导航" style={{ display: 'flex', gap: 6 }}>
          {filteredNav.map(({ path, label, icon: Icon }) => {
            const isRoleTab = ['/student', '/teacher', '/parent', '/admin'].includes(path);
            const isActive = isRoleTab
              ? (location.pathname === path || (path === '/student' && location.pathname === '/') || (path === '/parent' && location.pathname.startsWith('/parent/')))
        ...
        {/* 右侧：动效开关 + 用户 + 退出 */}
        <div className="lf-header-actions" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button
            onClick={toggle}
          ...
          {user && (
            <span className="lf-header-user" style={{
      ...
      <main className="lf-main-content" style={{ flex: 1, padding: '24px 16px', maxWidth: 1200, margin: '0 auto', width: '100%', position: 'relative', zIndex: 1 }}>
        <motion.div
