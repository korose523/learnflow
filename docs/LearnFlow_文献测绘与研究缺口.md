...
> **真实用户数据 = 0 行；685 项测试全部为单元与契约测试，无集成测试、无用户研究。**
...
- **与 LearnFlow 的差异（我们站在哪）**：
  ...
  - 关键启示：效应量**从认知到行为递减**（0.49 → 0.36 → 0.25），且**行为效应量的置信区间下限仅 0.04**。这意味着"游戏化能改变长期学习行为"的证据是弱的。LearnFlow 若声称"53 个游戏化机制提升学习投入"，**必须自己在行为层面测量**，不能靠引用元分析蒙混。
...
- **与 LearnFlow 的差异**：
  ...
  - **降维后的现实路径**：54 个机制已按语义归入 A–H 八个类别（8/8/6/10/10/4/4/3，含新增的 LF-M54，分布合计现为 54），类别级因子设计的交互空间降至 2^8，**在样本量上首次具备可行性**；机制级交互仍需稀疏假设。
...
- **与 LearnFlow 的差异（我们站在哪）**：
  ...
  - 它与本项目的机制治理结果形成**互证**：机制被大量实现后，即便全部接入运行时（本项目已从"48 个零调用"治理至"53/53 可达、孤儿 0"），其成熟度分布仍为完整 8 / 部分 37 / 占位 8，且仅 31 个具备独立运行时开关——**机制可达性的提升，并未自动带来机制有效性的证据**。这强化了"机制数量 ≠ 结果改善"这一判断。
...
| 指标 | 治理前基线 | 治理后现状 | 复算方式 |
|---|---:|---:|---|
| 四层计数口径分歧 | L0=81 / L1=60 / L2=53 / PRD 表格和=69 | 统一为 **L2=53** | `scripts/verify_counts.py` |
| 运行时可达机制 | 25 / 53（孤儿 28） | **53 / 53（孤儿 0）** | `scripts/scan_mechanism_landing.py` |
...
**可做的最小实证（极其重要）**：
1. 静态分析：对 53 个去重机制做调用图分析，量化"实现—调用"落差，并按机制类型（Sailer 分类学）与成熟度分层。
...
1. **G3-1 机制实现债及其治理**（★★★★★）—— **本项目就是数据集**。治理前 48/60 零调用、11 个内存容器、四层计数口径分歧（81/60/53/69）；治理后 53/53 可达、孤儿 0、接线强度可分层审计、决策快照可冻结。这是一份**带基线的纵向材料**，别人拿不到。**唯一门槛是必须做跨项目复制**（≥ 3 个外部系统）。
...
**为什么最锋利**：
1. **数据独有**：治理前的 28/53 孤儿机制、11 个内存容器、8 个 nudge 生成点中仅 1 个经仲裁 —— 这批**治理前后对照数据只有 LearnFlow 有**，任何竞争对手都拿不到，除非他们先建一个同样规模的生产系统然后自曝其短。
