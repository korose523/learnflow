...
> **真实用户数据 = 0 行；868 项测试（49 个测试文件）全部为单元与契约测试，无集成测试、无用户研究。**
...
### 3.x 学习成瘾化研究补充文献与班级宠物集成（LF-M54）

方向 ③ 原有「机制实现债 / 暗黑模式对立建模」两条主线。本轮新增锚定四本书的
23 条「学习成瘾化研究」文献测绘（见 `docs/LearnFlow_学习成瘾化研究_补充文献测绘.md`，
A 类心理学机制 8 + B 类游戏设计理论 9 + C 类实证 7），并已下沉到代码：

- **LAI（LF-M53）维度补强**：新增 `connection_quality`（Hari C1）、`incentive_sensitization`
  （A4）、`variable_ratio_exposure`（A8）、`hook_internal_trigger_dependency`（B4）四个文献参数，
  默认零惩罚、向后兼容。
- **MechanismArbitrator 风险自适应降权**：新增 `_ADDICTION_RISK` 与可选 `lai_risk`，
  LAI 风险高时冷却高成瘾化拉回型 nudge。
- **LF-M54 班级积分养宠系统 / 班级宠物园（第 54 个机制）**：每生一只专属电子宠物、用行为
  积分喂养并八级形态进化，班级层面聚合为「宠物园」（排行榜 / 凝聚力 / 每周仪式），对应 Hari C1 /
  Bartle-Yee 社交动机 / SDT relatedness；是方向 ③「抗成瘾干预」侧首个**以真实连接替代虚拟刺激的
  社交机制（带双刃剑护栏）**落地（PetProfile 复用 + ClassPetGarden 聚合 + 服务 + API），并把
  `connection_quality` 回灌 LAI。

=> 这三条把 G3-2 的「成瘾化劝服 vs 抗成瘾干预对立建模」从论证推进到**可运行代码**，
构成方向 ③ 论文「方法贡献」一节素材；机制总数 53 → 54，落地审计 orphan = 0 保持不变。
...
>游戏化的"有没有效"之争已由元分析终结：Sailer & Homner 2020 给出认知 g=0.49、动机 g=0.36、行为 g=0.25 的三级效应量，并识别出游戏叙事与社会互动为关键调节变量。单机制对心理需求的影响已由 Sailer 等 2017（N=419，2×2 设计）与 Mazarakis & Bräuer（N=505，四机制）完成。真空在两处：一是**生产级游戏化系统的机制实现债**——本项目已测得四层计数口径（L0 类 81 / L1 实现单元 60 / L2 语义去重 54 / PRD 表格求和 69），并在治理后达到 54/54 可达（孤儿 0），但**接线强度仍分两层**（34 个进入编排器步骤映射、31 个具备独立运行时开关），且成熟度分布为完整 9 / 部分 41 / 占位 4；二是**暗黑模式与健康目标的冲突建模**——系统内存在语义直接对立的干预（错失恐惧提示 vs 未成年保护 vs 休息提醒），三层漏斗仲裁器已实现并接入错失恐惧通道，其余劝导性消息生成点尚未纳入统一仲裁。
...
- **与 LearnFlow 的差异（我们站在哪）**：
  ...
  - 关键启示：效应量**从认知到行为递减**（0.49 → 0.36 → 0.25），且**行为效应量的置信区间下限仅 0.04**。这意味着"游戏化能改变长期学习行为"的证据是弱的。LearnFlow 若声称"54 个游戏化机制提升学习投入"，**必须自己在行为层面测量**，不能靠引用元分析蒙混。
...
- **与 LearnFlow 的差异（我们站在哪）**：
  ...
  - **但**：它只做了 **7 个机制**（点数、徽章、排行榜、表现图表、故事、头像、队友），且是**实验室模拟任务**（订单分拣），**不是真实长期学习系统**。LearnFlow 有 **54 个唯一机制**（四层计数口径：L0 类别计数 81 / L1 实现单元 60 / L2 语义去重 54 / PRD 表格求和 69；执行口径取 L2=54），是它的 **7.7 倍**，且是**生产系统、长期使用**。
...
- **与 LearnFlow 的差异**：
  - "组合不叠加"这一发现**直接威胁** LearnFlow 的 54 机制设计——如果 54 个机制的效果不叠加，那 54 个机制的意义何在？
  - **这正是 G3-3 的起点**：机制间的交互效应需要**因子设计（factorial design）**而非逐个 A/B。54 个机制的逐个交互空间是 2^54，**必须**用稀疏因子设计或序贯实验。这是一个**明确的方法论缺口**。
  - **降维后的现实路径**：54 个机制已按语义归入 A–H 八个类别（8/8/6/10/10/4/4/4，含新增的 LF-M54，分布合计现为 54），类别级因子设计的交互空间降至 2^8，**在样本量上首次具备可行性**；机制级交互仍需稀疏假设。
...
- **与 LearnFlow 的差异**：**这是一颗定时炸弹**。它意味着本项目花大力气实现的 54 个机制，其效果可能大部分来自"这个系统看起来像个游戏"这一**框架效应**，而非机制本身。
  - **负面影响**：若不回应，审稿人可用此文否定整个 54 机制工程。
...
- **与 LearnFlow 的差异（我们站在哪）**：
  - **这是方向 ③ 最重要的一篇**，因为 LearnFlow 的劝导性消息由**分散在各引擎中的多个 nudge 生成函数**产出，彼此之间没有统一的价值方向仲裁。
  - **实测口径（可一行命令复算）**：生产代码中 `def *nudge*` 形式的 nudge 生成函数共 **8 个**，分布于 `deep_addiction_engine`（3）、`gamification_service`（3）、`duolingo_addiction_engine`（1）、`positive_addiction_engine`（1）。其仲裁状态分三层：
    - **经统一仲裁器下发：1 个**——FOMO nudge（LF-M44）已改造为产出 `Effect` 候选，由 `MechanismArbitrator` 三层漏斗决定下发，未成年保护（LF-M52）命中时在第 1 层丢弃；
    - **运行时可达但未仲裁：2 个**——友好竞争 nudge（LF-M24）与预约提醒 nudge（LF-M41）已在编排器步骤 13 中被调用并记入决策快照，但**绕过仲裁层**；
    - **零调用：5 个**——同伴进度 nudge、任务 nudge、损失厌恶 nudge、班级掌握度 nudge、活跃学习者 nudge。
  - **因此"未仲裁"这一判断在 FOMO 上已被本项目自身修复**，但剩余 7 个（2 个可达未仲裁 + 5 个零调用）仍构成完整的审计对象。这一**治理前后对照**本身就是 G3-2 的一手材料。
...
- **与 LearnFlow 的差异（我们站在哪）**：
  ...
  - 它与本项目的机制治理结果形成**互证**：机制被大量实现后，即便全部接入运行时（本项目已从"48 个零调用"治理至"54/54 可达、孤儿 0"），其成熟度分布仍为完整 9 / 部分 41 / 占位 4，且仅 31 个具备独立运行时开关——**机制可达性的提升，并未自动带来机制有效性的证据**。这强化了"机制数量 ≠ 结果改善"这一判断。
...
**可验证性（LearnFlow 自身的一手数据，治理前后对照）**：

| 指标 | 治理前基线 | 治理后现状 | 复算方式 |
|---|---:|---:|---|
| 四层计数口径分歧 | L0=81 / L1=60 / L2=54 / PRD 表格和=69 | 统一为 **L2=54** | `scripts/verify_counts.py` |
| 运行时可达机制 | 25 / 54（孤儿 28） | **54 / 54（孤儿 0）** | `scripts/scan_mechanism_landing.py` |
| 具备独立运行时开关的机制 | 3 | **31**（另 3 个健康护栏豁免、19 个仅实现引用可达） | 编排器 `is_enabled` 字面量计数 |
| 内存态容器（进程重启丢状态） | 11 个裸字典 | **0 个裸字典**，全部经 `StateStore` 抽象，但**默认后端仍为内存实现** | `app/services/state_store.py` |
| 劝导消息统一仲裁 | 无 | **1 / 8 个生成点经仲裁器** | 见 N5 |
| 决策快照冻结与埋点 | 无 | **已具备**（`learning_events` 表 + JSON 快照） | `scripts/replay_telemetry.py --emit-events` |

**关键限定（不得省略）**：上表右侧的"治理后"仅说明**可达性与可审计性的提升**，**不构成任何有效性证据**。机制效果仍需在真实用户样本上测量后方可主张。
...
**可做的最小实证（极其重要）**：
1. 静态分析：对 54 个去重机制做调用图分析，量化"实现—调用"落差，并按机制类型（Sailer 分类学）与成熟度分层。
2. 根因分类：零调用的机制属于哪几类？（a）从未接入 UI；（b）接入但触发条件永不满足；（c）触发但无可见反馈；（d）被更晚实现的机制取代（重复实现）。**本项目治理前基线已给出该分类的实测分布，可作为方法学的标定案例。**
3. 后果量化：**治理前** 11 个内存容器在重启时丢失什么状态？用户可观测的后果是什么？**治理后**这些状态已迁移至可插拔持久化后端——须如实标注其默认后端仍为内存实现。
4. **跨项目复制**：单项目案例只能发 short paper。必须找 **≥ 3 个**其他开源游戏化学习系统做同样的静态分析，证明"机制实现债"是**普遍现象**而非本项目独有。**这一步是能否发到 CCF-B 及以上期刊的分水岭，不可省略。**
...
- **8 个 nudge 生成函数中仅 1 个经统一仲裁层**（FOMO，LF-M44）；2 个运行时可达但绕过仲裁（LF-M24、LF-M41），5 个零调用。多数劝导消息在生成后直接下发，未与健康目标做方向比对；
- 其中 FOMO 类 nudge（如"你的连续记录即将中断""XX 同学已经超过你"）**直接对抗** LAI（学习成瘾指数，0–100，五维加权）想要降低的成瘾行为。
- **本项目已实现的部分**：`MechanismArbitrator` 三层漏斗（健康一票否决 → 冲突消解保守偏置 → 干预预算），并已在 FOMO 通道上线。**未实现的部分**：其余 7 个生成点尚未纳入仲裁，仲裁器本身未在真实用户上做过效果评估。这两点必须在论文中如实区分。
...
**具体缺口**：54 个机制的**交互效应空间是 2^54**，逐个 A/B 不可行。文献中**没有**针对游戏化机制的**稀疏因子设计（sparse factorial design）**或**序贯实验（sequential experimental design）**方法论。
...
| "消融实验"这一想法本身 | Sailer 2017 与 Mazarakis 已做。我们必须说清楚：**我们的消融是 54 个机制、生产系统、且研究的是"实现债"而非"心理效应"** |

### 还活着的（按推荐度排序）—— **本方向是五个方向中 novelty 最有保障的**

1. **G3-1 机制实现债及其治理**（★★★★★）—— **本项目就是数据集**。治理前 48/60 零调用、11 个内存容器、四层计数口径分歧（81/60/54/69）；治理后 54/54 可达、孤儿 0、接线强度可分层审计、决策快照可冻结。这是一份**带基线的纵向材料**，别人拿不到。**唯一门槛是必须做跨项目复制**（≥ 3 个外部系统）。
...
**建议路线**：
- **第一步（立即）**：MSR 或 IST short paper —— 54 机制静态分析 + 实现债分类学（G3-1 单项目版，含治理前后对照）。
- **第二步（3–6 个月）**：EMSE / IST 完整版 —— 跨 ≥ 3 个开源游戏化学习系统的复制研究。
- **第三步（并行）**：CHI / Computers in Human Behavior —— 劝导性干预生成点的暗黑模式审计 + 与健康护栏的冲突检测（G3-2）。**这一条最有可能出 CCF-A。**
...
**为什么最锋利**：
1. **数据独有**：治理前的 28/54 孤儿机制、11 个内存容器、8 个 nudge 生成点中仅 1 个经仲裁 —— 这批**治理前后对照数据只有 LearnFlow 有**，任何竞争对手都拿不到，除非他们先建一个同样规模的生产系统然后自曝其短。
...
**一句话定位**：*"我们建了一个 54 机制的游戏化学习系统，然后审计它——发现 28 个机制从未被运行时调用、11 个内存容器在重启时丢失状态、8 个劝导点中只有 1 个经过与健康目标的方向仲裁。我们随后完成了治理并留下可复算的前后对照，这是行业普遍现象的第一个实证证据。"*
...
| **T2** 54 个游戏化机制的调用图静态分析 + 零调用根因分类 | 实现债分类学 | MSR / IST short paper |
| **T3** 8 个 nudge 生成点的 Gray 五分类双评审员编码 + 与 LAI 方向的余弦冲突检测 | 暗黑模式审计报告 | CHI / Computers in Human Behavior |
