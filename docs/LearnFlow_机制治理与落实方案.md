...
**问题一（计数不可复算）**：系统文档声称的机制数量与代码实际承载的机制数量不一致，且不一致的来源未被分解。在本文所研究的系统中，项目需求文档标题与概述声称 76 个机制，而同一文档 §2.3 的逐项表格求和仅为 69；经代码三层复算，语义去重后的唯一机制为 53。三个数字分别对应三种不同的计数口径，若不显式区分，论文中的任何数字陈述都可能被审稿人证伪。
...
1. **四层计数口径与其差异分解**：给出 L0（类名计数）→ L1（实现单元计数）→ L2（语义去重后唯一机制）的三层复算链，并显式分解 76 与 53 之间 23 的差额来源（§3）。
2. **预注册去重规则与敏感性区间**：提出 R1–R6 六条去重规则，报告 53 这一取值，并给出 51–56 的诚实区间（§4.2）。
...
系统需求文档（PRD）在标题与概述中声称"76 个游戏化成瘾引擎"，而同一文档 §2.3 的逐项表格求和为 69。二者之差为 7，属于文档内部矛盾。更严重的是，二者均高于代码可复算的唯一机制数 53。若不处理这一矛盾，任何引用 76 的论文陈述都可被审稿人在数分钟内证伪，进而动摇全文可信度。
...
| L2 | L1 经语义去重后的唯一机制构念数 | **53** | 本文采用的论文主表述 |
...
3. **7 = 60 − 53**：同一心理学构念存在多份重复实现，按预注册规则 R3 合并。

### 3.3 披露策略

本文采取**分层披露**策略：论文正文一律使用 53（唯一机制）并给出复算命令；在讨论项目历史文档时，明确写出"PRD 标题声称 76，与其 §2.3 表格求和 69 自相矛盾，经三层复算唯一机制为 53"，并给出上述差额分解。**禁止**在任何场合将 76 作为系统能力的正面陈述。
...
分类学需同时满足三个条件：**互斥**（每个机制归入唯一类别）、**完备**（53 个机制全部归类）、**理论可追溯**（每个类别对应明确的心理学理论传统，而非实现文件划分）。第三条尤其重要：按实现文件划分类别（如"深度成瘾引擎类"）虽有工程便利，但与理论构念无关，无法支持跨研究比较。
...
### 4.2 预注册去重规则与 53 的敏感性区间

53 这一取值依赖于以下六条预注册规则，规则必须在实验预注册中先行声明，否则该数字不可辩护。
...
**敏感性区间**：53 是上述规则下的结果。放宽 R2（将连胜神圣化并入连胜）得 52；收紧 R4（三个占位各计 1）得 55。因此**诚实区间为 51–56**，论文应报告 53 并同时给出该区间。

### 4.3 完整机制清单

**表 4-3　完整机制清单（LF-M01–LF-M53）**
...
> 复算命令：`python scripts/verify_counts.py` → `category_counts`（自检：类别和 53、编号连续无重复）
...
**发现一：实现成熟度分布严重右偏。** 53 个机制中仅 8 个（15.1%）达到完整成熟度，37 个（69.8%）处于"有逻辑但未持久化"状态，8 个（15.1%）为占位实现。这意味着在消融实验中，占多数的机制其处理效应可能弱到无法检出，且部分机制的处理变异本身可能不成立。
...
**发现三：健康护栏类（H）的成熟度与其重要性不匹配。** H 类 3 个机制中仅 1 个（LF-M52）达到完整成熟度，而该类机制在仲裁器中被赋予一票否决权（§5.4）。作为最高权限的干预，其实现完整性应优先于任何动机类机制。
...
落地扫描脚本（`scripts/scan_mechanism_landing.py`）对 53 个机制逐条判定两个布尔量：
...
机制被判为已落地（`landed`）当且仅当 `orchestrator_wired` 或 `external_ref` 为真。截至代码基线，`landed = 53`、`orphan = 0`。
...
| 指标 | 登记值 | 严格性 | 含义 |
|---|---:|---|---|
| `mechanism_unique` | 53 | 严格 | 唯一机制数（论文 Table 1 的数字） |
...
**缓解**：报告评分者间一致性 κ；对争议项给出单独列表；给出 51–56 的敏感性区间而非点估计。
...
**威胁**：部分机制实现成熟度为占位（8/53），其处理变异可能不成立。
...
| # | 项目 | 性质 | 对实验的影响 | 状态 |
|---:|---|---|---|---|
| 1 | `anti_addiction_compliance.py` 的 `MinorProtectionEngine`（LF-M52）**无调用方** | 合规硬伤 | 健康一票否决依赖编排器内构造的等效效应，而非该引擎本身 | 未实现 |
| 2 | `learning_orchestrator.py` 中 `get_skill_tree()` 返回值与 `_skilltree_repo_format()` 期望形状不兼容 | 既有缺陷 | 技能树相关结局在真实扁平入参下不可用 | 未修复 |
| 3 | `SQLExperimentStore` 未实现 | 基础设施缺口 | 实验数据仅支持 JSON 后端，规模化受限 | 未实现 |
| 4 | 9 个内存态容器默认仍为 `MemoryStateStore` | 持久化缺口 | 进程重启后状态丢失，影响长期结局的连续性 | 未实现 |
| 5 | 4 个机制（LF-M29、LF-M32、LF-M49、LF-M50）实现为占位 | 实现缺口 | 其步骤 13 输出为引擎占位数据，不参与主要分析 | 已标注 |
...
本文针对游戏化学习系统中机制计数不可复算、干预冲突无仲裁、接线状态不可审计这三个方法论缺陷，给出了一套可复现的处理方案：以四层计数口径与预注册去重规则确定 53 个唯一机制并给出 51–56 的敏感性区间；以统一效应接口与三层漏斗仲裁器确定性消解并发干预的方向性冲突；以编排器门控、落地扫描与计数印章使接线状态成为可机器验证的命题；以分层 Gatekeeping 与聚类随机化下的功效分析给出八类消融的实验设计。
...
| 注册表指纹 | `python -c "from app.services.mechanism_registry import registry_fingerprint as f; print(f())"` | `dc2b230762fc` |
| 成熟度分布 | `python -c "from app.services.mechanism_registry import all_mechanisms; import collections; print(collections.Counter(m.maturity for m in all_mechanisms()))"` | `partial=37, complete=8, placeholder=8` |
...
| 唯一机制 | unique mechanism | 经语义去重后的机制构念；本文主计数口径，取值 53 |
...
| 错误表述 | 正确表述 | 依据 |
|---|---|---|
| 76 个/种游戏化机制 | 53 个唯一机制（LF-M01–LF-M53）；PRD 标题声称 76 与其表格求和 69 自相矛盾 | §3.2，`verify_counts` |
| 23 / 21 种学习方法 | 28 种学习方法（LF-L01–LF-L28，权威源 `method_registry.py`） | `verify_counts` |
| 48 个零调用机制 | 落地 53/53，orphan = 0；其中 19 个仅有文本引用 | §6.4 |
| 394 个测试用例 | 685 passed（35 个测试文件） | `pytest tests/ -q` |
| 后端 16,593 行 | 后端 67 个 Python 文件 / 20,349 行 | 文件统计 |
