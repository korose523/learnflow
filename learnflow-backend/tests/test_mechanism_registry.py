"""游戏化机制注册表测试 — 54 个机制 (LF-M01..LF-M54) 的单一事实源完整性

覆盖目标:
1. 注册表是全量 54 个机制的单一事实源 (ID 连续无缺口、key 唯一、snake_case)
2. 每个机制都有必填治理字段 (theory_ref / impl_ref / stage / category / disposition)
3. 可按 stage / category / disposition 查询, 且各维度切分之和 == 54
...
class TestRegistryIntegrity:
    ...
    def test_keys_unique_and_snake_case(self, registry):
        ...
        assert len(set(all_keys)) == 54, "mechanism key 存在重复"
        for key in all_keys:
            assert key == key.lower() and key.replace("_", "").isalnum(), f"非 snake_case: {key}"

    def test_ids_contiguous_no_gap(self, registry):
        ids = registry.ids()
        assert len(set(ids)) == 54, "机制 ID 存在重复"
        assert ids == [f"LF-M{i:02d}" for i in range(1, 55)], f"ID 有缺口或错位: {ids}"
...
class TestDimensionSlices:
    def test_stage_slices_cover_all(self, registry):
        ...
        assert sum(len(b) for b in buckets) == 54, "stage 切分未覆盖全部机制"
        for spec in registry.all_mechanisms():
    ...
    def test_category_slices_cover_all(self, registry):
        ...
        assert sum(len(b) for b in buckets) == 54, "category 切分未覆盖全部机制"
        for spec in registry.all_mechanisms():
    ...
    def test_disposition_slices_cover_all(self, registry):
        ...
        assert sum(len(b) for b in buckets) == 54, "disposition 切分未覆盖全部机制"
        for spec in registry.all_mechanisms():
...
class TestToggle:
    def test_kill_switch(self, registry):
        ...
        try:
            ...
            assert len(registry.enabled_mechanisms()) == 53
        finally:
...
class TestDocCrossValidation:
    def test_registry_ids_match_governance_doc(self, registry):
        """注册表的 54 个 ID 必须与 docs/...机制治理方案.md §4.3 完整机制清单表完全一致。
