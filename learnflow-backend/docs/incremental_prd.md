...
| 文件数 | 收集测试数 | 状态 |
|--------|------------|------|
| 19 个测试文件 | 394 个（含 1 个收集错误） | 部分可用；`test_addiction_methods_v3.py` 因 `pydantic_settings` 版本冲突无法收集 |

> 文档声称 420 测试全部通过，实际当前环境无法完整运行，且测试数量有差异。
...
| 3.20 | **测试数量与文档不一致（394 vs 420）** | 回归覆盖不足 | 补齐 26 个缺失测试，重点覆盖 P0 新增逻辑 |
...
5. **测试目标**：是否需要补齐到 420 个测试并修复当前 `pydantic_settings` 依赖冲突，还是先保证 394 个可运行？
