...
def main():
    ...
    if code != 200:
        ...
        try:
            ...
            has_bkt = "bkt" in nt_data
            has_tip = bool(nt_data.get("learning_tip"))
            has_task = bool(nt_data.get("task"))
            log("Student /student/next-task task", has_task, detail=f"task_id={nt_data.get('task', {}).get('id')}")
            ...
            task_id = nt_data.get("task", {}).get("id")
        except Exception as e:
    ...
    if task_id:
        ...
        if code != 200:
            ...
            try:
                ...
                has_review = bool(ans_data.get("spaced_review"))
    ...
    if err:
        ...
        if code == 200:
            try:
                ...
                has_child = bool(ps_data.get("child") and ps_data["child"].get("name"))
                has_weekly = bool(ps_data.get("weekly"))
                log(f"Parent child summary student_id={sid}", has_child and has_weekly, detail=f"keys={list(ps_data.keys())}")
            except Exception as e:
